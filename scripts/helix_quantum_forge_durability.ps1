# HelixQuantum Forge durability proof — atomic guards, forced-kill survival, restore roundtrip
# Prereq: helix_quantum_forge_api on 8117 (already built), Postgres migrated,
#         docker compose postgres running, HELIX_ALLOW_DEV_HEADERS=1.
# The script force-kills and restarts the API mid-run.
# Override the binary path with HELIX_QUANTUM_FORGE_API_BIN if needed.

$ErrorActionPreference = "Stop"
$h = @{ "x-helix-dev-user" = "ops@helixforge.local"; "Content-Type" = "application/json" }
$base = "http://127.0.0.1:8117"
$Bin = if ($env:HELIX_QUANTUM_FORGE_API_BIN) { $env:HELIX_QUANTUM_FORGE_API_BIN } elseif ($IsWindows -or $env:OS -match "Windows") { "target/debug/helix_quantum_forge_api.exe" } else { "./target/debug/helix_quantum_forge_api" }
...
function Wait-Health($Seconds = 60) {
    ...
    throw "helix_quantum_forge_api did not become healthy"
}
...
Write-Host "=== job + circuit + submit + complete ==="
$suffix = Get-Random
$job = (InvokeApi POST "$base/v1/jobs" @{
    name = "Durability job $suffix"
    description = "gate proof"
}).data
$circuit = (InvokeApi POST "$base/v1/jobs/$($job.id)/circuits" @{
    title = "Gate circuit $suffix"
    body = "survives restart"
}).data
InvokeApi POST "$base/v1/jobs/$($job.id)/submit" | Out-Null
$completed = (InvokeApi POST "$base/v1/jobs/$($job.id)/complete").data
...
Write-Host "  job completed with circuit"

Write-Host "=== forced-kill proof: acknowledged completion survives ==="
Write-Host "  completed $($job.id), killing API..."
Get-Process -Name helix_quantum_forge_api -ErrorAction SilentlyContinue | Stop-Process -Force
Start-Sleep -Seconds 2
$check = $null
...
Wait-Health 60
Write-Host "  API back, verifying completed job and circuit..."
$survivor = (InvokeApi GET "$base/v1/jobs/$($job.id)").data
if ($survivor.status -ne "completed") { throw "expected completed after kill, got $($survivor.status)" }
if ($survivor.name -ne "Durability job $suffix") { throw "job name mismatch after kill" }
if (-not $survivor.completed_at) { throw "completed_at missing after kill" }
$circuits = (InvokeApi GET "$base/v1/jobs/$($job.id)/circuits").data.items
$crow = $circuits | Where-Object { $_.id -eq $circuit.id }
if (-not $crow) { throw "circuit missing after kill" }
if ($crow.title -ne "Gate circuit $suffix") { throw "circuit title mismatch after kill" }
if ($crow.status -ne "draft") { throw "expected draft circuit after kill, got $($crow.status)" }
Write-Host "  job and circuit fully present"

Write-Host "=== restore proof: quantum schema roundtrip ==="
$dumpPath = "/tmp/quantum_durability_dump.sql"
docker compose exec -T postgres pg_dump -U helix -d helixforge --schema=quantum --no-owner --no-privileges -f $dumpPath
...
Psql "postgres" "DROP DATABASE IF EXISTS quantum_restore_test" | Out-Null
Psql "postgres" "CREATE DATABASE quantum_restore_test" | Out-Null
docker compose exec -T postgres psql -U helix -d quantum_restore_test -f $dumpPath --quiet 2>$null | Out-Null

foreach ($table in @("jobs", "circuits")) {
    $src = Psql "helixforge" "SELECT COUNT(*) FROM quantum.$table"
    $dst = Psql "quantum_restore_test" "SELECT COUNT(*) FROM quantum.$table"
    if ($src -ne $dst) { throw "$table count mismatch: source=$src restored=$dst" }
}
$hashSql = "SELECT md5(COALESCE(string_agg(md5(id::text || ':' || title || ':' || status), '|' ORDER BY id), 'empty')) FROM quantum.circuits"
$srcHash = Psql "helixforge" $hashSql
$dstHash = Psql "quantum_restore_test" $hashSql
if ($srcHash -ne $dstHash) { throw "circuits content hash mismatch after restore" }
...
Write-Host ""
Write-Host "HELIX_QUANTUM_FORGE_DURABILITY PASS"
exit 0
